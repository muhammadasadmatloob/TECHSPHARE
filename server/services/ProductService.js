...
import { productTrie } from '../utils/ProductTrie.js';
import { apiCache } from '../utils/CacheManager.js';
import databaseService from '../config/DatabaseService.js';

// Initial Mock Seed Dataset for Fallback
...
class ProductService {
  constructor() {
    ...
    // Build initial DSA Trie Index
    productTrie.buildIndex(this.inMemoryProducts);
  }

  async getAllProducts(category = 'all', search = '') {
    const cacheKey = `products_${category}_${search}`;
    const cachedData = apiCache.get(cacheKey);
    if (cachedData) {
      return { products: cachedData, source: 'cache' };
    }

    let productsList = [];

    // Database lookup if connected
    if (databaseService.getStatus().isConnected) {
      try {
        ...
        productsList = await Product.find(query).lean();
      } catch (err) {
        console.warn('DB query error, using in-memory list fallback', err.message);
        productsList = this.inMemoryProducts;
      }
    } else {
      // In-Memory fallback lookup
      productsList = category === 'all' 
        ? this.inMemoryProducts 
        : this.inMemoryProducts.filter(p => p.category === category);
    }

    // DSA Trie Prefix Search Integration if search term provided
    if (search.trim()) {
      const trieMatches = productTrie.searchPrefix(search);
      const matchIds = new Set(trieMatches.map(m => m.id));
      productsList = productsList.filter(p => matchIds.has(p.id));
    }

    // Store in LRU Cache
    apiCache.set(cacheKey, productsList);

    return { products: productsList, source: 'live' };
  }
